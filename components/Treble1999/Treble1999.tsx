...
export function Treble1999() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto w-full h-full flex flex-col justify-between flex-1 relative">
        {/* Right Edge Vertical Progress Bar */}
        <div className="absolute -right-6 md:-right-10 top-1/4 bottom-1/4 w-[2px] bg-white/10 hidden sm:block">
          <div
            ref={progressBarRef}
            className="w-full h-full bg-flame origin-top scale-y-0"
          />
          <div className="absolute -right-8 top-0 font-mono text-[9px] text-flax-smoke-400 tracking-widest uppercase">
            01/03
          </div>
          <div className="absolute -right-8 bottom-0 font-mono text-[9px] text-flame tracking-widest uppercase font-bold">
            03/03
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          <div className="flex items-center gap-2 font-mono text-xs text-flame font-bold uppercase tracking-wider">
            <span>( CENTERPIECE // MAY 1999 )</span>
          </div>
            ...
            >
              {idx === 2 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center w-full">
                  <div className="lg:col-span-6 flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="font-display font-black text-3xl sm:text-4xl text-flame">
                        {stage.step}
                      </span>
                      <div className="font-mono text-[11px] tracking-widest text-flax-smoke-400 uppercase">
                        <span>ACT II &middot; TRIUMPH 03 OF 03 &middot; </span>
                        <span className="text-flax-smoke-100 font-bold">{stage.date}</span>
                      </div>
                    </div>

                    <h3 className="text-2xl sm:text-3xl md:text-4xl font-black font-display tracking-tight uppercase leading-tight text-flax-smoke-50">
                      {stage.trophy}
                    </h3>

                    <div className="my-3 p-3.5 bg-[#121211] border border-white/10 rounded-xl flex items-center justify-between">
                      <div className="font-display font-bold text-sm md:text-base text-flax-smoke-50">
                        vs {stage.opponent} &middot; <span className="text-flax-smoke-400 text-xs font-mono font-normal">{stage.venue}</span>
                      </div>
                      <div
                        className="font-display font-black text-xl text-flame"
                        dangerouslySetInnerHTML={{ __html: stage.scoreline }}
                      />
                    </div>

                    <p className="text-xs sm:text-sm text-flax-smoke-300 font-body leading-relaxed">
                      {stage.highlight}
                    </p>

                    <blockquote className="mt-3 font-mono text-xs text-flame italic tracking-wide">
                      {stage.quote}
                    </blockquote>
                  </div>
                ...
                <div className="w-full max-w-2xl flex flex-col justify-center">
                  ...
                  <h3 className="text-2xl sm:text-4xl md:text-5xl font-black font-display tracking-tight uppercase leading-tight text-flax-smoke-50">
                    {stage.trophy}
                  </h3>

                  <div className="my-4 p-4 bg-[#121211] border border-white/10 rounded-xl flex items-center justify-between">
                    <div className="font-display font-bold text-base md:text-lg text-flax-smoke-50">
                      vs {stage.opponent} &middot; <span className="text-flax-smoke-400 text-xs font-mono font-normal">{stage.venue}</span>
                    </div>
                    <div
                      className="font-display font-black text-xl md:text-2xl text-flame"
                      dangerouslySetInnerHTML={{ __html: stage.scoreline }}
                    />
                  </div>

                  <p className="text-xs sm:text-sm md:text-base text-flax-smoke-300 font-body leading-relaxed">
                    {stage.highlight}
                  </p>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-4 font-mono text-xs text-flax-smoke-400 tracking-widest uppercase">
          <span>( 3-STAGE PINNED PROGRESSION )</span>
          <span>( CONTINUE SCROLLING &darr; )</span>
        </div>
      </div>
    </section>
  );
